...
# #OSM Api
# osm_user = "abakus_uploader"
# osm_passwd = "$SAx&9Tx&4"
# osm_api_url = "https://api.openstreetmap.org"

#OSM Test API
osm_user = "abakus_imports"
osm_passwd = "PUbPymg8fC"
...
rep_street_sql = """
   ...
   select  
     ...
     array_agg('node/'||osm_id) as url, 
     array_agg(ST_Y(ST_Transform(way,4326))) as lat, 
     array_agg(ST_X(ST_Transform(way,4326))) as lon 
...
union all
   select  
     ...
     array_agg(ST_Y(ST_Transform(ST_Centroid(way),4326))) as lat, 
     array_agg(ST_X(ST_Transform(ST_Centroid(way),4326))) as lon 
